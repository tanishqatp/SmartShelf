...
Object detection is on-device and NPU-accelerated. Reasoning runs through Hermes Agent against a cloud-hosted model — a deliberate tradeoff explained in [Known limitations](#known-limitations) below, after an attempt to host the reasoning LLM on-device too ran into compatibility issues.
...
Copy the backroom-stock template and edit it to match your actual backroom counts (this drives the vendor-escalation logic — without it, `smart_shelf.py` still runs fine, it just can't tell whether backroom stock is low):
 
```bash
cp backroom_inventory.example.csv backroom_inventory.csv
```
 
Then open `smart_shelf.py` and fill in the placeholders near the top:
